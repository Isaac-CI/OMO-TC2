...
if __name__ == "__main__":
    ...
    for l1 in lambda_values:
        l2 = 1 - l1
        print(f"Processando lambda_1: {l1:.2f} - lambda_2: {l2:.2f} ...", end=" ")
        start_t = time.time()
        ...
        pareto_results.append({
            'lambda_1': l1,
            'J_WLS_Avg': total_wls / len(df),
            'J_L1_Avg': total_l1 / len(df)
        })
    ...
    if not df_pareto.empty:
        ...
        df_pareto['WLS_Norm'] = df_pareto['J_WLS_Avg'] / denom_wls
        df_pareto['L1_Norm'] = df_pareto['J_L1_Avg'] / denom_l1
        ...
        plt.xlabel('Custo Estatístico Médio Normalizado (WLS)')
        plt.ylabel('Custo de Massa Médio Normalizado (L1)')
        plt.legend()
        plt.grid(True, linestyle='--', alpha=0.5)
        
        # Margem
        plt.xlim(0, 1.005*max_wls)
        plt.ylim(0, 1.005*max_l1)
